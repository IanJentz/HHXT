...
initial = [basename,'_solu']; % steady state results filename, used as initial conditions
filename = [basename,'_trans']; % transient results filename
...
switch flowBC
    case 'DP'
        ...
        applyStreamBoundaryCondition(model,'Edge',H_edges_in,'Stream',2,'Direction','inlet',...
            'Pressure',P_H_in,'Temperature',@T_H_in_trans);
...
t_steps = [0,10];
t_steps = unique([0:65,65:5:120]);
load(initial,'results');
setInitialConditions(model,results);

if verbose == true
disp('starting transient solution :...')    
end

model.SolverOptions.AbsoluteTolerance = 1e0;

%run the transient solution, and time and display progress
if verbose == true
tic;
disp('    ...solving step')
end
results = solveHHXTpde(model,t_steps); % solve the model
if verbose == true
solve_time = toc;
finish_date = datestr(now);
disp(['finished transient solution at ',finish_date,' elapsed time ',num2str(solve_time),' sec'])
end
...
%% Transient Boundary Condition Functions
% Functions defining the transient boundary conditions
% Functional definitions must accept the inputs (region,state)
% where 

function val = T_H_in_trans(region,state)
    global T_H_in
    val_start = T_H_in; % ramp starting value
    t_start = 2; % ramp start time in sec
    DELTA_val = -60; % ramp magnitude
    DELTA_t = 60; % ramp time in sec
    t = state.time;
    
    % return value based on t in the ramp
    t_range =  [ -inf,0 ; 0,DELTA_t ; DELTA_t,inf ]+t_start; 
    val_range = val_start + [ 0.000  ; (t-t_start)/DELTA_t ; 1   ]*DELTA_val;
    val = val_range(sum(t <= t_range,2) == 1);
    if isnan(t); val = nan; end
end
