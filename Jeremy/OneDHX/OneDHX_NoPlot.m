...
function OneDHX_NoPlot(m_dot_c, run_file_name)
%% Define how this script runs
% These variables will change how this modeling script runs.  The model can
% be either a counter flow or parrallel flow Heat Exchanger.  CO2
% properties can be constant, isobaric (at 27.5 MPa isobar), or real gas.
% Flow boundary conditions can be choose to be either a Direchlet condition
% of an applied pressure drop, or Neuman conditions of applied mass flow or
% mass flux.  Finally the treatment of Darcy friction factor and Nusselt
% number can either be set constant or take functional definitions as
...
m_dot_C = str2num(m_dot_c); %converting str to num
...
switch fNumode
    case 'const'
        ...
        fmatr = [f_D;100*f_D]; % direction friction factor (channels are in x direction)
        Nu_C = Nu;  % Nusselt on cold side
        Nu_H = Nu;  % Nusselt on hot side
    case 'Re dependent'
        fmatr = @ff_D; % Colebrook friction factor (Moody Chart)
...
% the cold stream is stream 1 and we specify it on the only face in the
% model, face 1
...
% the hot stream is stream 2 and we specify it on the only face in the
% model, face 1
...
% side switches direction based on the choice of flowDir.
H_edge_in = 4; % choose edge 4 as the hot inlet
H_edge_out = 2; % choose edge 2 as hot outlet
switch flowDir % change the choice of edge for the cold side inlet and outlets
    case 'counter'
        C_edge_in = 2;
        C_edge_out = 4;
    case 'parallel'
...
% plot temperatures
PlotObj.updatePlot('Temperatures','Wireframe','on','TempLimits',[T_C_in,T_H_in]);
...
PlotObj.updatePlot('FluidHeating','Wireframe','on');
PlotObj.updatePlot('FluidHeating','Wireframe','on','CLimits',1e8*[-1,1]); % fix the limits of the color bar
...
PlotObj.updatePlot('Press+ChannelVelocityVector','Wireframe','on','pdeplotArgs',{'AutoScaleFactor',0.5,'Color','magenta'});
...
PlotObj.updatePlot('BodyTemperature','Mesh','on','TempLimits',[T_C_in,T_H_in]);
...
%% Plot the centerline results
% Plot the results as interpolated at the y = 0 centerline.

% this is implemented in the OneDHX_PlotResults.m in order to save space
% OneDHX_PlotResults; Testing speed in CHTC -Jeremy
