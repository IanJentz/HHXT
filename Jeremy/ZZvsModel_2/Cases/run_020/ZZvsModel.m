function [rmsd_fiber,rmsd_TC,rmsd_DP,rmsd_Tout,rmsd_qdot,Re_C,Re_H,DP_C,DP_H,T_C_out,T_H_out,qdot_C,qdot_H,Re_C_std,Re_H_std] = ZZvsModel(f_D,j_C_C,j_C_H)
%ZZVSMODEL runs the ZZ_Recuperator.m model and compares the result to
%          experimental data given in HX_state.mat.
%   Inputs:
%     j_C - Colburn heat transfer coefficient JEREMY: modified input
%     statement to suit input.txt format
...
%MODIFICATIONS to optimize for CHTC -Jeremy
f_D = str2num(f_D)
j_C_C = str2num(j_C_C)
j_C_H = str2num(j_C_H)
j_C = [j_C_C, j_C_H];
...
Re_C_std = std(results.Reynolds(results.Reynolds(:,1)~=0,1))
Re_H = results.Tables{1}.Reynolds(2) % hot side Reynolds number
Re_H_std = std(results.Reynolds(results.Reynolds(:,2)~=0,2))
