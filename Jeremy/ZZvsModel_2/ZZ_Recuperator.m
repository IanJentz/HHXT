function [model,results] = ZZ_Recuperator(opr_dir,varargin)

%% Parse function input, Run Specification

if mod(length(varargin),2)
    error('missmatched inputs')
end

HX_state =[];
f_D = 0.181; j_C = 8e-3;
BCSpec = 'mdot';%'DP' or 'mdot'
propmode = 'const';%'isobar' or 'const' or 'real'
...
if isempty(HX_state); load('HX_state.mat','HX_state'); end
...
basename = 'ZZ_Recupe_2D_Results';
filename = [dir_save,'\',basename,'_solu.mat'];

warning off

%% Parse BC conditions from HX_State 
%HX geom
L = 22.7338*25.4e-3; W = 4.6774*25.4e-3; H = 14*1.5e-3; %33e-3;
W_head = 3.3865*25.4e-3;
W_chan = 3.6777*25.4e-3;

%cold side conditions
switch HX_state.m_dot_C.unit
    case 'kg/h'
        conv = 1/3600;
    case 'kg/s'
        conv = 1;
end
m_dot_C = HX_state.m_dot_C.mean*conv;      %mass flow in kg/s 
T_C_in = HX_state.T_C_in.mean;        %temp in C
T_C_out = HX_state.T_C_out.mean;      %temp in C
switch HX_state.P_C_in.unit
    case 'psi'
        conv = 6894.76;
    case 'Pa'
        conv = 1;
end
P_C_in = HX_state.P_C_in.mean*conv;    %press in Pa
switch HX_state.DP_C.unit
    case 'psi'
        conv = 6894.76;
    case 'Pa'
        conv = 1;
end
DP_C = HX_state.DP_C.mean*conv;        %press drop in Pa
P_C_out = P_C_in - DP_C; 
G_C_in = m_dot_C/(W_head*H); %mass flux in kg/m2-s
G_C_out = m_dot_C/(W_head*H); %mass flux in kg/m2-s

%cold side inlet/outlets
C_in = 38;
C_out = 31;

%hot side conditions
switch HX_state.m_dot_H.unit
    case 'kg/h'
        conv = 1/3600;
    case 'kg/s'
        conv = 1;
end
m_dot_H = HX_state.m_dot_H.mean*conv;      %mass flow in kg/s 
T_H_in = HX_state.T_H_in.mean;        %temp in C
T_H_out = HX_state.T_H_out.mean;      %temp in C
switch HX_state.P_H_in.unit
    case 'psi'
        conv = 6894.76;
    case 'Pa'
        conv = 1;
end
P_H_in = HX_state.P_H_in.mean*conv;    %press in Pa
switch HX_state.DP_H.unit
    case 'psi'
        conv = 6894.76;
    case 'Pa'
        conv = 1;
end
DP_H = HX_state.DP_H.mean*conv;        %press drop in Pa
P_H_out = P_H_in - DP_H; 
G_H_in = m_dot_H/(W_head*H); %mass flux in kg/m2-s
G_H_out = m_dot_H/(W_head*H); %mass flux in kg/m2-s

%hot side inlet/outlets
H_in = 37;
H_out = 32;
...
model = createHHXT(N); %constructor for the HHXT environment
...
D_h = 0.996418e-3;  % hydraulic diameter of channels in m
% D_h = 2*D_h;
p = 3.282e-3; % channel pitch in m
th_p = 1.5e-3; % plate thickness in m
phi_core = 0.263285; % volume fraction of channels in the core
phi_C = (7/14)*phi_core;
phi_H = (6/14)*phi_core;

% global kfact_core
c_kfact_core = [0.2663285, 0.447333, 0.589462];
...
switch propmode
    ...
    case 'const'
        ...
        c_k316_S = c_kfact*c_k;

        c_kfact = (13*c_kfact_core([1,2])+1*[1,1])/14; % not using z dimension
        c_kfact = diag(c_kfact); c_kfact = c_kfact(:);
        c_k316_ChHh = c_kfact*c_k;

        c_kfact = (7*c_kfact_core([1,2])+6*c_kfact_core([2,1])+1*[1,1])/14; % not using z dimension
        c_kfact = diag(c_kfact); c_kfact = c_kfact(:);
        c_k316_ChHv = c_kfact*c_k;

        c_kfact = (7*c_kfact_core([2,1])+6*c_kfact_core([1,2])+1*[1,1])/14; % not using z dimension
        c_kfact = diag(c_kfact); c_kfact = c_kfact(:);
        c_k316_CvHh = c_kfact*c_k;

        c_kfact = (13*c_kfact_core([2,1])+1*[1,1])/14; % not using z dimension
        c_kfact = diag(c_kfact); c_kfact = c_kfact(:);
        c_k316_CvHv = c_kfact*c_k;

        c_kfact = (7*c_kfact_core([2,1])+7*[1,1])/14; % not using z dimension
        c_kfact = diag(c_kfact); c_kfact = c_kfact(:);
        c_k316_Cv = c_kfact*c_k;

        c_kfact = (6*c_kfact_core([2,1])+8*[1,1])/14; % not using z dimension
        c_kfact = diag(c_kfact); c_kfact = c_kfact(:);
        c_k316_Hv = c_kfact*c_k;
        
        sld1 = HHXTProperties(model,'Face',1,'Stream',0,...
            'PhaseFraction',1,...
            'ThermalConductivity',c_k316_S,...
            'MassDensity',8030,...
            'SpecificHeat',502);

        sld2 = HHXTProperties(model,'Face',4,'Material',sld1,...
            'PhaseFraction',(1-phi_C-phi_H),...
            'ThermalConductivity',c_k316_ChHh);
        sld = HHXTProperties(model,'Face',3,'Material',sld2,...
            'ThermalConductivity',c_k316_CvHv);
        HHXTProperties(model,'Face',5,'Material',sld);
        sld = HHXTProperties(model,'Face',2,'Material',sld2,...
            'ThermalConductivity',c_k316_ChHv);
        HHXTProperties(model,'Face',11,'Material',sld);
        sld = HHXTProperties(model,'Face',6,'Material',sld2,...
            'ThermalConductivity',c_k316_CvHh);
        HHXTProperties(model,'Face',12,'Material',sld);            
        sld = HHXTProperties(model,'Face',7,'Material',sld1,...
            'PhaseFraction',(1-phi_H),...
            'ThermalConductivity',c_k316_Hv);
        HHXTProperties(model,'Face',10,'Material',sld);    
        sld = HHXTProperties(model,'Face',8,'Material',sld1,...
            'PhaseFraction',(1-phi_C),...
            'ThermalConductivity',c_k316_Cv);
        HHXTProperties(model,'Face',9,'Material',sld);
    ...
    case {'isobar','real'}
        ...
        sld1 = HHXTProperties(model,'Face',1,'Stream',0,...
            'PhaseFraction',1,...
            'ThermalConductivity',@k316_S,...
            'MassDensity',8030,...
            'SpecificHeat',502);

        sld2 = HHXTProperties(model,'Face',4,'Material',sld1,...
            'PhaseFraction',(1-phi_C-phi_H),...
            'ThermalConductivity',@k316_ChHh);
        sld = HHXTProperties(model,'Face',3,'Material',sld2,...
            'ThermalConductivity',@k316_CvHv);
        HHXTProperties(model,'Face',5,'Material',sld);
        sld = HHXTProperties(model,'Face',2,'Material',sld2,...
            'ThermalConductivity',@k316_ChHv);
        HHXTProperties(model,'Face',11,'Material',sld);
        sld = HHXTProperties(model,'Face',6,'Material',sld2,...
            'ThermalConductivity',@k316_CvHh);
        HHXTProperties(model,'Face',12,'Material',sld);            
        sld = HHXTProperties(model,'Face',7,'Material',sld1,...
            'PhaseFraction',(1-phi_H),...
            'ThermalConductivity',@k316_Hv);
        HHXTProperties(model,'Face',10,'Material',sld);    
        sld = HHXTProperties(model,'Face',8,'Material',sld1,...
            'PhaseFraction',(1-phi_C),...
            'ThermalConductivity',@k316_Cv);
        HHXTProperties(model,'Face',9,'Material',sld);  
...
switch propmode
    ...
    case 'const'
        ...
        fldC = HHXTProperties(model,'Face',4,'Stream',1,...
            'PhaseFraction',phi_C,...
        ...
        fldH = HHXTProperties(model,'Face',4,'Stream',2,...
            'PhaseFraction',phi_H,...
            'ThermalConductivity',c_k,...       %thermal conductivity in W/m-K
            'MassDensity',c_rho,...     %densitiy in kg/m3
            'SpecificHeat',c_cp,...      %heat capacity in J/kg-K
    ...
    case 'isobar'
        ...
        fldC = HHXTProperties(model,'Face',4,'Stream',1,...
            'PhaseFraction',phi_C,...
            'ThermalConductivity',@CO2_k,...       %thermal conductivity in W/m-K
            'MassDensity',@CO2_rho,...     %densitiy in kg/m3
            'SpecificHeat',@CO2_cp,...      %heat capacity in J/kg-K
            'HydraulicDiameter',D_h,...    %hydraulic diameter in m
            'Viscosity',@CO2_mu,...      %viscosity in kg/m-s
            'jColburn',j_C_C,...
            'Nusselt',@Nu_C,...
            'fDarcy',fmatrh,...
            'Resistance',R_V_cond); 
...
HHXTProperties(model,'Face',11,'Material',fldC);    
fldC = HHXTProperties(model,'Face',3,'Material',fldC,...
    'fDarcy',fmatrv);
HHXTProperties(model,'Face',5,'Material',fldC); 
HHXTProperties(model,'Face',6,'Material',fldC);
HHXTProperties(model,'Face',8,'Material',fldC);
HHXTProperties(model,'Face',9,'Material',fldC);
HHXTProperties(model,'Face',12,'Material',fldC);

% define hot stream (CO2) material
HHXTProperties(model,'Face',6,'Material',fldH);    
HHXTProperties(model,'Face',12,'Material',fldH);    
fldH = HHXTProperties(model,'Face',2,'Material',fldH,...
    'fDarcy',fmatrv);
HHXTProperties(model,'Face',3,'Material',fldH); 
HHXTProperties(model,'Face',5,'Material',fldH);
HHXTProperties(model,'Face',7,'Material',fldH);
HHXTProperties(model,'Face',10,'Material',fldH);
HHXTProperties(model,'Face',11,'Material',fldH); 
...
%% Init Step: simple conductive PDE with Dirichlet BCs
test = true;
...
%% Sol Step: common BCs and coefficients
...
switch BCSpec
    ...
    case 'DP'
        %cold stream
        applyStreamBoundaryCondition(model,'Edge',C_in,'Stream',1,'Direction','inlet',...
            'Pressure',P_C_in,'Temperature',T_C_in);
        applyStreamBoundaryCondition(model,'Edge',C_out,'Stream',1,'Direction','outlet',...
            'Pressure',P_C_out);
        %hot stream
        applyStreamBoundaryCondition(model,'Edge',H_in,'Stream',2,'Direction','inlet',...
            'Pressure',P_H_in,'Temperature',T_H_in);
        applyStreamBoundaryCondition(model,'Edge',H_out,'Stream',2,'Direction','outlet',...
            'Pressure',P_H_out);
        
    case 'mflux'
        %cold stream
        applyStreamBoundaryCondition(model,'Edge',C_in,'Stream',1,'Direction','inlet',...
            'Pressure',P_C_in,'Temperature',T_C_in,'MassFlux',G_C_in);
        applyStreamBoundaryCondition(model,'Edge',C_out,'Stream',1,'Direction','outlet',...
            'MassFlux',G_C_out);
        %hot stream
        applyStreamBoundaryCondition(model,'Edge',H_in,'Stream',2,'Direction','inlet',...
            'Pressure',P_H_in,'Temperature',T_H_in,'MassFlux',G_H_in);
        applyStreamBoundaryCondition(model,'Edge',H_out,'Stream',2,'Direction','outlet',...
            'MassFlux',G_H_out);
...
% solver options
model.MatrixOptions = {'Permeability','calculated','ResistanceMode',resistmode, ...
...
if plotting == true
PlotResults(filename,dir_save,breakpoints);
end
...
function k = k316(temp)
    [~,nr] = size(temp);
    k = 17.6; % 316H at 250 C
%     k = interp1([20;100;200;300;400;5000;600;700;750]',[14.1;15.4;16.8;18.3;19.7;21.2;22.6;23.9;24.6]',temp);
    k = k*ones(1, nr); 
    k(isnan(temp)) = nan; 
end

function kmatrix = k316_S(pos,time,temp)
    kfact = [1;0;0;1]; % not using z dimension
    k = k316(temp);
    kmatrix = kfact*k;
end

function kmatrix = k316_ChHh(pos,time,temp)
%     global kfact_core
    kfact_core = [0.2663285, 0.447333, 0.589462];
    kfact = (13*kfact_core([1,2])+1*[1,1])/14; % not using z dimension
    kfact = diag(kfact); kfact = kfact(:);
    k = k316(temp);
    kmatrix = kfact*k;
end

function kmatrix = k316_ChHv(pos,time,temp)
%     global kfact_core
    kfact_core = [0.2663285, 0.447333, 0.589462];
    kfact = (7*kfact_core([1,2])+6*kfact_core([2,1])+1*[1,1])/14; % not using z dimension
    kfact = diag(kfact); kfact = kfact(:);
    k = k316(temp);
    kmatrix = kfact*k;
end

function kmatrix = k316_CvHh(pos,time,temp)
%     global kfact_core
    kfact_core = [0.2663285, 0.447333, 0.589462];
    kfact = (7*kfact_core([2,1])+6*kfact_core([1,2])+1*[1,1])/14; % not using z dimension
    kfact = diag(kfact); kfact = kfact(:);
    k = k316(temp);
    kmatrix = kfact*k;
end

function kmatrix = k316_CvHv(pos,time,temp)
%     global kfact_core
    kfact_core = [0.2663285, 0.447333, 0.589462];
    kfact = (13*kfact_core([2,1])+1*[1,1])/14; % not using z dimension
    kfact = diag(kfact); kfact = kfact(:);
    k = k316(temp);
    kmatrix = kfact*k;
end

function kmatrix = k316_Cv(pos,time,temp)
%     global kfact_core
    kfact_core = [0.2663285, 0.447333, 0.589462];
    kfact = (7*kfact_core([2,1])+7*[1,1])/14; % not using z dimension
    kfact = diag(kfact); kfact = kfact(:);
    k = k316(temp);
    kmatrix = kfact*k;
end

function kmatrix = k316_Hv(pos,time,temp)
%     global kfact_core
    kfact_core = [0.2663285, 0.447333, 0.589462];
    kfact = (6*kfact_core([2,1])+8*[1,1])/14; % not using z dimension
    kfact = diag(kfact); kfact = kfact(:);
    k = k316(temp);
    kmatrix = kfact*k;
end
...
function Nu = Nu_C(pos,time,Re,Pr)
    nRe = -1.73; %-1.0; %0.8
    Nu = C_Nu*Re.^nRe.*Pr.^0.4;
end

function Nu = Nu_H(pos,time,Re,Pr)
    nRe = -1.73; %-1.0; %0.8
    Nu = C_Nu*Re.^nRe.*Pr.^0.3;
end

%% CO2 Properties
% Constant Pressure Properties (27.5 MPa)

function rho = CO2_rho(pos,time,temp,press)
    %for a 13.8 MPa isobar
    ...
    y = [894.400000000000;808.600000000000;694.500000000000;542;410.600000000000;335.800000000000;290.800000000000;260.300000000000;237.800000000000;220.300000000000;206;194.100000000000;183.900000000000;175.100000000000;167.300000000000;160.400000000000;154.200000000000;148.500000000000;143.400000000000;138.700000000000;134.300000000000;130.300000000000;126.500000000000;123;119.700000000000;116.700000000000;113.800000000000;111;108.400000000000;106;103.600000000000;101.400000000000;99.3100000000000;97.3000000000000;95.3700000000000;93.5200000000000;91.7600000000000;90.0600000000000;88.4300000000000;86.8700000000000;85.3600000000000;83.9200000000000;82.5200000000000;81.1700000000000;79.8700000000000;78.6200000000000;77.4000000000000;76.2300000000000;75.0900000000000;73.9900000000000;72.9200000000000];
...
function mu = CO2_mu(pos,time,temp,press)
    %for a 13.8 MPa isobar
    T = [20;33.6000000000000;47.2000000000000;60.8000000000000;74.4000000000000;88;101.600000000000;115.200000000000;128.800000000000;142.400000000000;156;169.600000000000;183.200000000000;196.800000000000;210.400000000000;224;237.600000000000;251.200000000000;264.800000000000;278.400000000000;292;305.600000000000;319.200000000000;332.800000000000;346.400000000000;360;373.600000000000;387.200000000000;400.800000000000;414.400000000000;428;441.600000000000;455.200000000000;468.800000000000;482.400000000000;496;509.600000000000;523.200000000000;536.800000000000;550.400000000000;564;577.600000000000;591.200000000000;604.800000000000;618.400000000000;632;645.600000000000;659.200000000000;672.800000000000;686.400000000000;700];
    y = [9.01100000000000e-05;7.23900000000000e-05;5.57500000000000e-05;4.03800000000000e-05;3.11400000000000e-05;2.74100000000000e-05;2.58200000000000e-05;2.51000000000000e-05;2.48200000000000e-05;2.47800000000000e-05;2.48800000000000e-05;2.50800000000000e-05;2.53500000000000e-05;2.56700000000000e-05;2.60200000000000e-05;2.64000000000000e-05;2.68000000000000e-05;2.72100000000000e-05;2.76300000000000e-05;2.80500000000000e-05;2.84900000000000e-05;2.89300000000000e-05;2.93700000000000e-05;2.98100000000000e-05;3.02600000000000e-05;3.07000000000000e-05;3.11400000000000e-05;3.15900000000000e-05;3.20300000000000e-05;3.24700000000000e-05;3.29100000000000e-05;3.33500000000000e-05;3.37800000000000e-05;3.42100000000000e-05;3.46400000000000e-05;3.50700000000000e-05;3.54900000000000e-05;3.59200000000000e-05;3.63400000000000e-05;3.67500000000000e-05;3.71700000000000e-05;3.75800000000000e-05;3.79900000000000e-05;3.83900000000000e-05;3.87900000000000e-05;3.91900000000000e-05;3.95900000000000e-05;3.99900000000000e-05;4.03800000000000e-05;4.07700000000000e-05;4.11500000000000e-05];
    val = interp1(T,y,temp);
    val(temp<T(1)) = y(1); val(temp>T(end)) = y(end);
    mu = val;%0*val+0.000035;
end

function k = CO2_k(pos,time,temp,press)
    %for a 13.8 MPa isobar
    T = [20;33.6000000000000;47.2000000000000;60.8000000000000;74.4000000000000;88;101.600000000000;115.200000000000;128.800000000000;142.400000000000;156;169.600000000000;183.200000000000;196.800000000000;210.400000000000;224;237.600000000000;251.200000000000;264.800000000000;278.400000000000;292;305.600000000000;319.200000000000;332.800000000000;346.400000000000;360;373.600000000000;387.200000000000;400.800000000000;414.400000000000;428;441.600000000000;455.200000000000;468.800000000000;482.400000000000;496;509.600000000000;523.200000000000;536.800000000000;550.400000000000;564;577.600000000000;591.200000000000;604.800000000000;618.400000000000;632;645.600000000000;659.200000000000;672.800000000000;686.400000000000;700];
    y = [0.103000000000000;0.0875600000000000;0.0739300000000000;0.0605900000000000;0.0489400000000000;0.0421700000000000;0.0388100000000000;0.0371700000000000;0.0364300000000000;0.0362200000000000;0.0363100000000000;0.0366000000000000;0.0370000000000000;0.0376800000000000;0.0384300000000000;0.0392200000000000;0.0400400000000000;0.0408800000000000;0.0417500000000000;0.0426300000000000;0.0435300000000000;0.0444300000000000;0.0453400000000000;0.0462600000000000;0.0471900000000000;0.0481100000000000;0.0490400000000000;0.0499700000000000;0.0509000000000000;0.0518300000000000;0.0527600000000000;0.0536900000000000;0.0546200000000000;0.0555500000000000;0.0564700000000000;0.0574000000000000;0.0583200000000000;0.0592300000000000;0.0601500000000000;0.0610600000000000;0.0619700000000000;0.0628800000000000;0.0637800000000000;0.0646800000000000;0.0655800000000000;0.0664800000000000;0.0673700000000000;0.0682600000000000;0.0691400000000000;0.0700200000000000;0.0709000000000000];
    val = interp1(T,y,temp);
    val(temp<T(1)) = y(1); val(temp>T(end)) = y(end);
    k = val;%0*val+0.055;
end
...
function cp = CO2_cp(pos,time,temp,press)
    %for a 13.8 MPa isobar
    T = [20;33.6000000000000;47.2000000000000;60.8000000000000;74.4000000000000;88;101.600000000000;115.200000000000;128.800000000000;142.400000000000;156;169.600000000000;183.200000000000;196.800000000000;210.400000000000;224;237.600000000000;251.200000000000;264.800000000000;278.400000000000;292;305.600000000000;319.200000000000;332.800000000000;346.400000000000;360;373.600000000000;387.200000000000;400.800000000000;414.400000000000;428;441.600000000000;455.200000000000;468.800000000000;482.400000000000;496;509.600000000000;523.200000000000;536.800000000000;550.400000000000;564;577.600000000000;591.200000000000;604.800000000000;618.400000000000;632;645.600000000000;659.200000000000;672.800000000000;686.400000000000;700];
    y = [2307;2633;3267;3918;3130;2348;1930;1693;1545;1447;1378;1329;1292;1265;1244;1228;1216;1206;1199;1194;1191;1189;1187;1187;1187;1188;1189;1190;1192;1194;1197;1199;1202;1205;1208;1210;1213;1217;1220;1223;1226;1229;1232;1235;1238;1241;1244;1247;1250;1253;1256];
    val = interp1(T,y,temp);
    val(temp<T(1)) = y(1); val(temp>T(end)) = y(end);
    cp = val;%0*val+1600;
end

% Varying Pressure and Temperature Properties
